...
#pragma omp parallel for private(d)
    ...
    {
      typedef void(*void_function_pointer_type)(void);

      const void_function_pointer_type pf_proc = (const void_function_pointer_type) proc;

      for(i = 0; i < nthreads; ++i)
      {
        d[i].min        = i * block_size;
        d[i].max        = d[i].min + block_size;
        d[i].thread_num = i;
        d[i].data       = data;

        fftw_thread_spawn(&tid[i],
                          (fftw_thread_function) pf_proc,
                          (void*) &d[i]);
      }
    }
